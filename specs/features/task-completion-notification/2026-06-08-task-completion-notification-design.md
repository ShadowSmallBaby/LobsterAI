...
- 通知标题建议为 `任务已完成`。
- 通知正文包含会话标题或可识别的任务摘要，例如 `Cowork 会话已完成`。
- 通知点击后：
  - 恢复并聚焦主窗口。
  - 切换到 Cowork 页面。
  - 打开对应 session。
- 通知不得包含敏感输出内容、完整提示词、token 或本地路径等隐私信息。
...
- 用户聚焦窗口但没有查看对应 session 时，不自动清除该 session 的提醒。
...
- 推荐新增 IPC：
  - `cowork:markSessionViewed`
  - `cowork:openSessionFromNotification`
...
- 应用从后台恢复且 Cowork 当前 session 未变化时，再次上报当前 session 已查看。
